...
int main() {
    ...
    Tensor *y = tensor_conv2d(x,w,NULL);
    /* Expected output shape [1,1,2,2]; compute expected values manually */
    /* positions:
    ...
    assert(y->data[0]==6.0f);
    assert(y->data[1]==8.0f);
    assert(y->data[2]==12.0f);
    assert(y->data[3]==14.0f);
    ...
    */
    ...
    tensor_free(x); tensor_free(w); tensor_free(y); tensor_free(s);
    free(in_shape); free(w_shape);
