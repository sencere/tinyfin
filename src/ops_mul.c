...
static Tensor *reduce_to_target(const Tensor *grad_out, const Tensor *target) {
    ...
    if (!g) return NULL;
    /* If autograd is enabled, allow this reduced grad tensor to record grads
       (preparation for higher-order gradients). */
...
/* -------------------- forward / backward for mul -------------------- */

static void mul_fwd(Tensor *a, Tensor *b, Tensor *out) {
    int *out_shape = NULL;
    int out_ndim = 0;
...
static void mul_bwd(AutogradNode *n) {
    Tensor *out = n->out;
    if (!out || !out->grad) return;

        (void)out; /* debug removed */
    ...
    for (size_t i = 0; i < out->size; i++) {
        ...
        if (gA_bcast) gA_bcast->data[i] = go * eb.data[ib];
        if (gB_bcast) gB_bcast->data[i] = go * ea.data[ia];
    }
